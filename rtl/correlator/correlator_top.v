...
module tart_correlator #(
    ...
    input           clock_x_i,  // correlator clock
    input           reset_x_i,  // correlator reset (todo)?

    input           clock_b_i,  // SRAM/bus clock
    input           reset_b_i,  // SRAM/bus reset

    //  AXI4 Stream interface for reading visibilities:
    output          tvalid_o,   // indicates unread visibities
    input           tready_i,   // downstream able to receive
    output          tlast_o,    // last transfer of frame
    output [WSB:0]  tdata_o,    // visibilities data

    //  AXI4 Stream providing raw signal IQ data
    input           rvalid_i,   // asserts that signal data is valid
    output          rready_o,   // when able to receive data 'frame'
    input           rlast_i,    // indicates end-of-frame
    input  [ISB:0]  rdataq_i,   // oversampled source 'Q' signal
    input  [ISB:0]  rdatai_i,   // oversampled source 'I' signal
